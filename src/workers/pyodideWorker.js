...
ensurePyodideReady().then(() => {
  //console.log('[worker] Pyodide initialized, sending initial ready');
  self.postMessage({ type: 'ready' });
});

const loadRequiredPackages = async (packages = []) => {
  for (const pkg of packages) {
    //console.log('[worker] Pakcage:', pkg);
...
self.onmessage = async (event) => {
  ...
  //console.log('[worker] Received message:', type, code, packages);
